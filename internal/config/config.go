...
func Setup() {
	...
	log.Printf("AwsRegion, %v", Config.AwsRegion)
	log.Printf("AwsAPIEndpoint, %v", Config.AwsAPIEndpoint)
	log.Printf("S3Bucket, %v", Config.S3Bucket)
	log.Printf("S3KeyPrefix, %v", Config.S3KeyPrefix)
	log.Printf("IndexDocument, %v", Config.IndexDocument)
	log.Printf("DirectoryListing, %v", Config.DirectoryListing)
	log.Printf("DirListingFormat, %v", Config.DirListingFormat)
	log.Printf("HTTPCacheControl, %v", Config.HTTPCacheControl)
	log.Printf("HTTPExpires, %v", Config.HTTPExpires)
	log.Printf("BasicAuthUser, %v", Config.BasicAuthUser)
	log.Printf("BasicAuthPass, %v", Config.BasicAuthPass)
	log.Printf("Port, %v", Config.Port)
	log.Printf("Host, %v", Config.Host)
	log.Printf("AccessLog, %v", Config.AccessLog)
	log.Printf("SslCert, %v", Config.SslCert)
	log.Printf("SslKey, %v", Config.SslKey)
	log.Printf("StripPath, %v", Config.StripPath)
	log.Printf("ContentEncoding, %v", Config.ContentEncoding)
	log.Printf("CorsAllowOrigin, %v", Config.CorsAllowOrigin)
	log.Printf("CorsAllowMethods, %v", Config.CorsAllowMethods)
	log.Printf("CorsAllowHeaders, %v", Config.CorsAllowHeaders)
	log.Printf("CorsMaxAge, %v", Config.CorsMaxAge)
	log.Printf("HealthCheckPath, %v", Config.HealthCheckPath)
	log.Printf("AllPagesInDir, %v", Config.AllPagesInDir)
	log.Printf("MaxIdleConns, %v", Config.MaxIdleConns)
	log.Printf("IdleConnTimeout, %v", Config.IdleConnTimeout)
	log.Printf("DisableCompression, %v", Config.DisableCompression)
	log.Printf("InsecureTLS, %v", Config.InsecureTLS)
	log.Printf("JwtSecretKey, %v", Config.JwtSecretKey)
}
